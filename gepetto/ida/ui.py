...
from gepetto.models.base import GPT4_MODEL_NAME, GPT3_MODEL_NAME,SPARK_MODEL_NAME
...
class GepettoPlugin(idaapi.plugin_t):
    ...
    select_spark_menu_path = "Edit/Gepetto/" + _("Select model") + f"/{SPARK_MODEL_NAME}"
    ...
    def generate_plugin_select_menu(self):
        ...
        idaapi.unregister_action(self.select_spark_action_name)
        ...
        idaapi.detach_action_from_menu(self.select_spark_menu_path, self.select_gpt4_action_name)
        ...
        self.select_spark_action_name = f"gepetto:{''.join(random.choices(string.ascii_lowercase, k=7))}"
        ...
        # Select spark action
        select_spark_action = idaapi.action_desc_t(self.select_spark_action_name,
                                                  SPARK_MODEL_NAME,
                                                  None if str(gepetto.config.model) == SPARK_MODEL_NAME
                                                  else SwapModelHandler(SPARK_MODEL_NAME, self),
                                                  "",
                                                  "",
                                                  208 if str(gepetto.config.model) == SPARK_MODEL_NAME else 0)
        idaapi.register_action(select_spark_action)
        idaapi.attach_action_to_menu(self.select_gpt35_menu_path, self.select_spark_action_name, idaapi.SETMENU_APP)
